...
export default function SignIn(props) {
  ...
  return (
    <form onSubmit={props.account} id="login">
      <Link to="/">
        <img style={{ width: 120, filter: "drop-shadow(2px 2px 1px #00000077)" }} src="img/logo/chatty-logo-nobg-round.jpg" alt="" />
      </Link>
      <div className="signin">
        ...
        <button id="submitbtn" disabled={props.isButtonDisabled} type="submit">
          Continue
        </button>
      </div>
      {props.purpose === "Sign in" ? (
        <Link className="create-button" to="/signup">
          Create your Chatty account
        </Link>
      ) : (
